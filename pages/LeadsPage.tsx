...
export const LeadsPage: React.FC = () => {
  ...
  useEffect(() => {
     if (currentUser?.org_id) {
        fetchData();
    } else if (!currentUser && isDataLoading === false) { // Ensure it only runs once if no currentUser
        setPageLoading(false); // Stop loading if no user/org context
    }
  ...
  const handleSaveLead = async (leadFormData: Omit<Lead, 'id' | 'created_at' | 'updated_at' | 'owner_name' | 'org_id'> | Lead) => {
    ...
    // Modal has its own loading state, setPageLoading for table refresh indication.
    // setPageLoading(true); 
    try {
      ...
      // setPageLoading(false); // Only if error stops further operation
    }
    // Page loading will be handled by fetchData's finally block
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            />
             <Select
                ...
                placeholder="All Statuses" // This will be the default option label
            />
            <Input
                name="source"
