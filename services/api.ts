  ...
  public: {
    Tables: {
      ...
      deals: {
          ...
          Update: Partial<Omit<Deal, 'id' | 'created_at' | 'updated_at' | 'created_by_user_id' | 'created_by_user_name' | 'company_name' | 'contact_name' | 'lead_name' | 'items' | 'org_id' | 'lead_id'>>
      }
      deal_items: {
          Row: DealItem
      ...
      lead_follow_ups: {
          ...
          Update: Partial<Omit<LeadFollowUp, 'id' | 'created_at' | 'updated_at' | 'user_full_name' | 'lead_id' | 'org_id'>>
      }
      tasks: {
          Row: Task
      ...
      tickets: {
          ...
          Update: Partial<Omit<Ticket, 'id' | 'ticket_uid' | 'created_at' | 'updated_at' | 'org_id' | 'created_by_user_id' | 'created_by_user_name' | 'assigned_to_user_name'>>
      }
    }
    Views: {
      [_ in never]: never
  ...
  registerAdminAndOrganization: async (registrationData: AdminRegistrationData): Promise<{ authUser: SupabaseAuthUser, organization: Organization, profile: UserProfile }> => {
    ...
    try {
      const { data: orgData, error: orgError } = await supabase
        .from('organizations')
        .insert({ name: registrationData.organizationName } as any)
        .select()
        .single();
      ...
      const { data: profileData, error: profileError } = await supabase
        .from('users')
        .insert(userProfilePayload as any)
        .select()
        .single();
...
export const userService = {
  ...
  addUser: async (userData: { email: string; passwordInput: string; full_name: string; role: Role; role_name?: string | null; parent_user_id?: string | null }, adminOrgId: string): Promise<UserProfile> => {
    ...
    const { data: profileData, error: profileError } = await supabase
      .from('users')
      .insert(newUserProfileData as any)
      .select()
      .single();
    if (profileError) {
  ...
  deleteUser: async (userId: string, authUserId: string): Promise<void> => {
    ...
    await supabase.from('users').update({ parent_user_id: null } as any).eq('parent_user_id', userId);
...
export const leadFollowUpService = {
  ...
  updateFollowUp: async (
    ...
    const payload: Partial<LeadFollowUp> = { ...updateData };
...
export const productService = {
  ...
  updateProduct: async (productId: string, productData: Partial<Omit<Product, 'id' | 'created_at' | 'updated_at' | 'org_id'>>): Promise<Product> => {
    const payload: Database['public']['Tables']['products']['Update'] = { ...productData };
    const { data, error } = await supabase
      .from('products')
      .update(payload as any)
      .eq('id', productId)
...
export const companyService = {
  ...
  updateCompany: async (companyId: string, companyData: Partial<Omit<Company, 'id' | 'created_at' | 'updated_at' | 'org_id'>>): Promise<Company> => {
    const payload: Database['public']['Tables']['companies']['Update'] = { ...companyData };
    const { data, error } = await supabase.from('companies').update(payload as any).eq('id', companyId).select().single();
    if (error) throw error;
  ...
  deleteCompany: async (companyId: string): Promise<void> => {
    ...
    await supabase
        .from('leads')
        .update({ company_id: null, contact_id: null } as any)
        .eq('company_id', companyId);
...
export const contactService = {
  ...
  addContact: async (contactData: Omit<Contact, 'id' | 'created_at' | 'updated_at' | 'org_id' | 'company_name'>, orgId: string): Promise<Contact> => {
    ...
    const { data, error } = await supabase.from('contacts').insert(payload as any).select('*, company:companies(name)').single();
    if (error) throw error;
  ...
  updateContact: async (contactId: string, contactData: Partial<Omit<Contact, 'id' | 'created_at' | 'updated_at' | 'org_id' | 'company_name'>>): Promise<Contact> => {
    const payload: Database['public']['Tables']['contacts']['Update'] = { ...contactData };
    const { data, error } = await supabase.from('contacts').update(payload as any).eq('id', contactId).select('*, company:companies(name)').single();
    if (error) throw error;
...
export const dealService = {
  ...
  addDeal: async (
    ...
    if (dealItemsToInsert.length > 0) {
      const { error: itemsError } = await supabase.from('deal_items').insert(dealItemsToInsert as any);
      if (itemsError) {
  ...
  updateDeal: async (
    ...
    const totalValue = itemsData.reduce((sum, item) => sum + (item.quantity * item.unit_price), 0);
    const dealPayload: Database['public']['Tables']['deals']['Update'] = { ...dealData, total_value: totalValue };
...
export const taskService = {
    ...
    updateTask: async(taskId: string, taskData: Partial<Omit<Task, 'id'|'created_at'|'updated_at'|'org_id'|'created_by_user_id'|'created_by_user_name'|'assigned_to_user_name'|'comments' | 'related_lead_name' | 'related_company_name' | 'related_contact_name'>>): Promise<Task> => {
        const payload: Database['public']['Tables']['tasks']['Update'] = {...taskData};
    ...
    addTaskComment: async(commentData: Omit<TaskComment, 'id'|'created_at'|'user_full_name'|'org_id'|'user_id'>, orgId: string, userId: string): Promise<TaskComment> => {
        ...
        const { data, error } = await supabase.from('task_comments').insert(payload as any).select().single();
        if (error) throw error;
...
export const ticketService = {
  ...
  addTicket: async(ticketData: Omit<Ticket, 'id'|'ticket_uid'|'created_at'|'updated_at'|'org_id'|'created_by_user_id'|'created_by_user_name'|'assigned_to_user_name'|'resolved_at'|'closed_at'>, orgId: string, creatorUserId: string): Promise<Ticket> => {
    ...
    const { data, error } = await supabase.from('tickets').insert(payload as any).select().single();
    if(error) throw error;
  ...
  updateTicket: async(ticketId: string, ticketData: Partial<Omit<Ticket, 'id'|'ticket_uid'|'created_at'|'updated_at'|'org_id'|'created_by_user_id'|'created_by_user_name'|'assigned_to_user_name'>>, originalTicket?: Ticket | null): Promise<Ticket> => {
    ...
    const { data, error } = await supabase.from('tickets').update(payload as any).eq('id', ticketId).select().single();
    if (error) throw error;
