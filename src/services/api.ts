...
export const reportService = {
  getLeadStats: async (orgId: string): Promise<SalesReportData> => {
    ...
    const relevantLeadsForConversion = totalLeads - newLeadsCount; // Exclude 'New' leads from conversion base if that's the logic
    ...
    const { data: allDeals, error: dealsErrorFetch } = await supabase
        .from('deals')
        .select('status, total_value')
        .eq('org_id', orgId);

    if (dealsErrorFetch) {
        console.error("Error fetching deals for stats:", dealsErrorFetch);
        throw dealsErrorFetch;
    }

    const dealsByStatusMap = new Map<DealStatus, { count: number, total_value: number }>();
    (allDeals || []).forEach(deal => {
        const status = deal.status as DealStatus;
        const current = dealsByStatusMap.get(status) || { count: 0, total_value: 0 };
        current.count += 1;
        current.total_value += deal.total_value;
        dealsByStatusMap.set(status, current);
    });
    ...
    return {
        totalLeads, convertedLeadsCount, lostLeadsCount, newLeadsCount, contactedLeadsCount, qualifiedLeadsCount,
        conversionRate, leadsByStatus, leadsBySource,
        totalDeals, totalWonDealsValue, dealsByStatus: dealsByStatusResult,
    };
  },
  ...
  getDealPageReportData: async (orgId: string, currentUserId: string, currentUserRole: Role): Promise<DealPageReportData> => {
    ...
    if (currentUserRole !== Role.ADMIN) {
        query = query.eq('created_by_user_id', currentUserId); // Filter for user's deals if not admin
    }
    ...
    (deals || []).forEach(deal => {
      totalDealsCount++;
      const status = deal.status as DealStatus;
      const value = deal.total_value;

      const currentMapStats = dealsByStatusMap.get(status) || { count: 0, total_value: 0 };
      currentMapStats.count++;
      currentMapStats.total_value += value;
      dealsByStatusMap.set(status, currentMapStats);

      if (status === DealStatus.WON) {
        wonDealsValue += value;
        wonDealsCount++;
      } else if (status !== DealStatus.LOST && status !== DealStatus.CANCELLED) {
        openDealsValue += value;
      }
    });

    const dealsByStatusResult: DealsByStatusReportItem[] = Array.from(dealsByStatusMap.entries())
    ...
    return {
      ...
      openDealsValue,
      wonDealsValue,
      avgWonDealSize: wonDealsCount > 0 ? wonDealsValue / wonDealsCount : 0,
  ...
  getTaskDashboardStats: async (orgId: string, currentUserId: string): Promise<TaskDashboardStats> => {
        const { data: tasks, error } = await supabase
            ...
            .eq('org_id', orgId)
            .eq('assigned_to_user_id', currentUserId); // Only tasks assigned to the current user
        ...
        tasks.forEach(task => {
            ...
            tasksByPriorityMap.set(priority, (tasksByPriorityMap.get(priority) || 0) + 1);
        ...
        return {
            ...
            openTasks,
            overdueTasks,
            tasksByStatus: Array.from(tasksByStatusMap.entries()).map(([status, count]) => ({status, count})),
            tasksByPriority: Array.from(tasksByPriorityMap.entries()).map(([priority, count]) => ({priority, count})),
        };
    }
};
...
export const taskService = {
  getTasks: async (orgId: string, currentUserId: string, currentUserRole: Role, allOrgUsers: UserProfile[]): Promise<Task[]> => {
    let query = supabase
      .from('tasks')
      .select(`*, 
    ...
    return (data || []).map(task => {
        ...
        return {
            ...
            assigned_to_user_name: assignedToUser?.full_name || allOrgUsers.find(u => u.id === task.assigned_to_user_id)?.full_name || 'N/A',
            created_by_user_name: createdByUser?.full_name || allOrgUsers.find(u => u.id === task.created_by_user_id)?.full_name || 'N/A',
            ...
            assigned_to: undefined, created_by: undefined, lead: undefined, company: undefined, contact: undefined, 
        } as Task;
    });
  },
  ...
  addTask: async (taskData: Omit<Task, 'id' | 'created_at' | 'updated_at' | 'org_id' | 'created_by_user_id' | 'created_by_user_name' | 'assigned_to_user_name' | 'comments' | 'related_lead_name' | 'related_company_name' | 'related_contact_name'>, orgId: string, creatorUserId: string): Promise<Task> => {
    ...
    // Denormalized names will be populated by getTasks or a getTaskById if needed
    return data as Task; 
  },
  ...
  deleteTask: async (taskId: string): Promise<void> => {
    await supabase.from('task_comments').delete().eq('task_id', taskId); // Delete comments first
  ...
  addTaskComment: async (commentData: Omit<TaskComment, 'id' | 'created_at' | 'user_full_name' | 'org_id' | 'user_id'>, orgId: string, userId: string): Promise<TaskComment> => {
  ...
  deleteTasksForRelatedEntity: async (relationField: 'related_lead_id' | 'related_company_id' | 'related_contact_id', entityId: string): Promise<void> => {
    ...
    if (fetchError) {
        console.error(`Failed to fetch tasks related to ${relationField} ${entityId}:`, fetchError);
        return;
    }

    if (tasksToDelete && tasksToDelete.length > 0) {
        const taskIds = tasksToDelete.map(t => t.id);
        // Delete comments for these tasks first
        ...
        if (deleteError) {
            console.error(`Failed to delete tasks related to ${relationField} ${entityId}:`, deleteError);
        }
    }
  },
};
...
export const ticketService = {
  getTickets: async (orgId: string, currentUserId: string, currentUserRole: Role): Promise<Ticket[]> => {
    ...
    return (data || []).map(ticket => {
        const assignedToUser = ticket.assigned_to as unknown as { full_name: string } | null;
        const createdByUser = ticket.created_by as unknown as { full_name: string } | null;
        return {
            ...ticket,
            status: ticket.status as TicketStatus,
            priority: ticket.priority as TicketPriority,
            assigned_to_user_name: assignedToUser?.full_name || 'Unassigned',
  ...
  addTicket: async (ticketData: Omit<Ticket, 'id' | 'ticket_uid' | 'created_at' | 'updated_at' | 'org_id' | 'created_by_user_id' | 'created_by_user_name' | 'assigned_to_user_name' | 'resolved_at' | 'closed_at'>, orgId: string, creatorUserId: string): Promise<Ticket> => {
    const ticket_uid = `TCK-${Date.now().toString().slice(-6)}`;
    const payload = { ...ticketData, ticket_uid, org_id: orgId, created_by_user_id: creatorUserId };
  ...
  updateTicket: async (ticketId: string, ticketData: Partial<Omit<Ticket, 'id' | 'ticket_uid' | 'created_at' | 'updated_at' | 'org_id' | 'created_by_user_id' | 'created_by_user_name' | 'assigned_to_user_name' | 'resolved_at' | 'closed_at'>>, existingTicket?: Ticket | null): Promise<Ticket> => {
    const payload: any = { ...ticketData, updated_at: new Date().toISOString() };

    if (existingTicket) {
        if (ticketData.status === TicketStatus.RESOLVED && existingTicket.status !== TicketStatus.RESOLVED) {
            payload.resolved_at = new Date().toISOString();
        } else if (ticketData.status !== TicketStatus.RESOLVED && ticketData.status !== TicketStatus.CLOSED && existingTicket.status === TicketStatus.RESOLVED) {
             payload.resolved_at = null; 
        }
        if (ticketData.status === TicketStatus.CLOSED && existingTicket.status !== TicketStatus.CLOSED) {
            payload.closed_at = new Date().toISOString();
            if (existingTicket.status !== TicketStatus.RESOLVED) payload.resolved_at = new Date().toISOString(); 
        } else if (ticketData.status !== TicketStatus.CLOSED && existingTicket.status === TicketStatus.CLOSED) {
