  ...
  public: {
    Tables: {
      ...
      leads: {
        ...
        Update: Partial<Omit<Lead, 'id' | 'created_at' | 'owner_name' | 'company_name' | 'contact_name' | 'activities' | 'follow_ups' | 'deals' | 'org_id'>>
      }
      products: {
        Row: Product
        Insert: Omit<Product, 'id' | 'created_at' | 'updated_at'>
        Update: Partial<Omit<Product, 'id' | 'created_at' | 'org_id'>>
      }
      companies: {
          Row: Company
          Insert: Omit<Company, 'id' | 'created_at' | 'updated_at'>
          Update: Partial<Omit<Company, 'id' | 'created_at' | 'org_id'>>
      }
      contacts: {
          Row: Contact
          Insert: Omit<Contact, 'id' | 'created_at' | 'updated_at' | 'company_name'>
          Update: Partial<Omit<Contact, 'id' | 'created_at' | 'org_id' | 'company_name'>>
      }
      deals: {
          Row: Deal
      ...
      tasks: {
          ...
          Update: Partial<Omit<Task, 'id' | 'created_at' | 'org_id' | 'created_by_user_id' | 'assigned_to_user_name' | 'created_by_user_name' | 'related_lead_name' | 'related_company_name' | 'related_contact_name' | 'comments'>>
      }
      task_comments: {
          Row: TaskComment
...
export const leadService = {
  ...
  updateLead: async (leadId: string, leadData: Partial<Omit<Lead, 'id' | 'created_at' | 'updated_at' | 'owner_name' | 'company_name' | 'contact_name' | 'org_id' | 'activities' | 'follow_ups' | 'deals'>>): Promise<Lead> => {
    const payload = { ...leadData, updated_at: new Date().toISOString() };
...
export const dealService = {
  ...
  updateDeal: async (
    ...
    const totalValue = itemsData.reduce((sum, item) => sum + (item.quantity * item.unit_price), 0);
    const dealPayload = { ...dealData, total_value: totalValue, updated_at: new Date().toISOString() };
...
export const ticketService = {
  ...
  updateTicket: async(ticketId: string, ticketData: Partial<Omit<Ticket, 'id'|'ticket_uid'|'created_at'|'updated_at'|'org_id'|'created_by_user_id'|'created_by_user_name'|'assigned_to_user_name'>>, originalTicket?: Ticket | null): Promise<Ticket> => {
    const payload: Partial<Ticket> = {...ticketData, updated_at: new Date().toISOString()};
