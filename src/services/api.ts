...
import { SUPABASE_URL, SUPABASE_ANON_KEY } from '@/constants'; // This import ensures the check in constants.ts runs
...
export const reportService = {
  ...
  getTaskDashboardStats: async (orgId: string, userId: string, userRole: Role): Promise<TaskDashboardStats> => {
    // This function would typically call an RPC in Supabase.
    // For client-side only, we'd fetch all tasks and filter/aggregate.
    // Returning default/empty data for now to ensure buildability.
    ...
    return {
        ...
        openTasks: 0,
        overdueTasks: 0,
        tasksByStatus: TASK_STATUS_OPTIONS.map(status => ({ status, count: 0 })),
        tasksByPriority: TASK_PRIORITY_OPTIONS.map(priority => ({ priority, count: 0 })),
    };
  },

  getDealPageReportData: async (orgId: string, userId: string, userRole: Role): Promise<DealPageReportData> => {
    // This function would ideally call an RPC.
    // Returning default/empty data for now.
    ...
    return {
        ...
        wonDealsValue: 0,
        avgWonDealSize: 0,
        dealsByStatus: DEAL_STATUS_OPTIONS.map(status => ({ status, count: 0, total_value: 0 })),
    };
  }
};
...
export const taskService = {
  getTasks: async (orgId: string, currentUserId: string, currentUserRole: Role, allOrgUsers: UserProfile[]): Promise<Task[]> => {
    ...
    // If user is not admin, filter by created_by_user_id or assigned_to_user_id
    if (currentUserRole !== Role.ADMIN) {
  ...
  addTask: async (taskData: Omit<Task, 'id' | 'created_at' | 'updated_at' | 'org_id' | 'created_by_user_id' | 'created_by_user_name' | 'assigned_to_user_name' | 'comments' | 'related_lead_name' | 'related_company_name' | 'related_contact_name'>, orgId: string, createdByUserId: string): Promise<Task> => {
    ...
    // Fetch again to get joined names, or construct locally if all user/lead/etc data is available
    return this.getTaskDetails(data.id, orgId);
  },
  ...
  updateTask: async (taskId: string, taskData: Partial<Omit<Task, 'id' | 'created_at' | 'updated_at' | 'org_id' | 'created_by_user_id' | 'created_by_user_name' | 'assigned_to_user_name' | 'comments' | 'related_lead_name' | 'related_company_name' | 'related_contact_name'>>): Promise<Task> => {
    const payload: any = { ...taskData, updated_at: new Date().toISOString() };
     // Ensure null is sent for empty optional foreign keys
  ...
  getTaskComments: async (taskId: string, orgId: string): Promise<TaskComment[]> => {
    const { data, error } = await supabase
      ...
      .eq('task_id', taskId)
      // .eq('org_id', orgId) // Assuming task_comments table also has org_id or relies on RLS via task_id
  ...
  addTaskComment: async (commentData: Omit<TaskComment, 'id' | 'created_at' | 'user_full_name'>, orgId: string, userId: string): Promise<TaskComment> => {
    const payload = { ...commentData, user_id: userId /*, org_id: orgId */ }; // Add org_id if table has it
...
export const ticketService = {
    ...
    updateTicket: async (ticketId: string, updates: Partial<Omit<Ticket, 'id' | 'ticket_uid' | 'created_at' | 'updated_at' | 'org_id' | 'created_by_user_id' | 'created_by_user_name' | 'assigned_to_user_name' | 'resolved_at' | 'closed_at'>>, originalTicket: Ticket): Promise<Ticket> => {
        const payload: any = { ...updates, updated_at: new Date().toISOString() };
        if (updates.status && updates.status !== originalTicket.status) {
            if (updates.status === TicketStatus.RESOLVED && !updates.resolved_at) payload.resolved_at = new Date().toISOString();
            else if (updates.status === TicketStatus.CLOSED && !updates.closed_at) payload.closed_at = new Date().toISOString();
            else if (updates.status !== TicketStatus.RESOLVED) payload.resolved_at = null;
            else if (updates.status !== TicketStatus.CLOSED) payload.closed_at = null;
        }
         if (updates.hasOwnProperty('assigned_to_user_id')) {
            payload.assigned_to_user_id = updates.assigned_to_user_id || null;
        }
    ...
    deleteTicket: async (ticketId: string): Promise<void> => {
        // Consider deleting related comments or other associated data if necessary
