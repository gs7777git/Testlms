  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={contactToEdit ? "Edit Contact" : "Add New Contact"} size="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Select label="Company" id="company_id" name="company_id" value={contactData.company_id} onChange={handleChange} options={companyOptions} error={errors.company_id} required disabled={isLoadingModal || companies.length === 0} placeholder={companies.length === 0 ? "No companies available" : "-- Select Company --"} />
...
export const ContactsPage: React.FC = () => {
  ...
  const fetchData = useCallback(async (options?: {isInitialLoad?: boolean}) => {
    ...
    try {
      const promises: (Promise<Contact[]> | Promise<Company[]>)[] = [contactService.getContacts(currentUserProfile.org_id)];
      if (isInitialLoad) {
  ...
  const filteredContacts = useMemo(() => {
    return contacts.filter(contact => {
        ...
        const searchMatch = 
            `${contact.first_name} ${contact.last_name}`.toLowerCase().includes(searchTermLower) ||
            (contact.email_primary && contact.email_primary.toLowerCase().includes(searchTermLower)) ||
            (contact.company_name && contact.company_name.toLowerCase().includes(searchTermLower)) ||
            (contact.designation && contact.designation.toLowerCase().includes(searchTermLower));
        
        const companyMatch = filterCompany ? contact.company_id === filterCompany : true;
        
        return searchMatch && companyMatch;
    });
  }, [contacts, searchTerm, filterCompany]);
  
  const companyFilterOptions = [{value: '', label: 'All Companies'}, ...companies.map(c => ({value: c.id, label: c.name}))];
  ...
  return (
    <div>
      ...
      {companies.length === 0 && !pageLoading && !isDataLoading && (
         <p className="mb-4 p-3 bg-yellow-50 border border-yellow-200 text-yellow-700 rounded-md text-sm">
            Please add companies first to be able to associate contacts. <a href="/companies" className="font-semibold hover:underline">Go to Companies</a>.
        </p>
      )}
        ...
        <table className="min-w-full divide-y divide-secondary-200" aria-label="Contacts Table">
          ...
          <tbody className="bg-white divide-y divide-secondary-200">
            {isDataLoading && contacts.length > 0 ? (
              ...
              <tr><td colSpan={7} className="px-6 py-12 text-center text-sm text-secondary-500">No contacts found.</td></tr>
            ) : (
              filteredContacts.map((contact) => (
                <tr key={contact.id} className="hover:bg-secondary-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-secondary-500">{new Date(contact.updated_at).toLocaleDateString()}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                    <Button variant="outline" size="sm" onClick={() => handleOpenModal(contact)} aria-label={`Edit ${contact.first_name} ${contact.last_name}`}><EditIcon className="h-4 w-4" /></Button>
                    {hasRole(Role.ADMIN) && 
                        <Button variant="danger" size="sm" onClick={() => handleDeleteContact(contact.id)} aria-label={`Delete ${contact.first_name} ${contact.last_name}`}><DeleteIcon className="h-4 w-4" /></Button>
                    }
                  </td>
                </tr>
              ))
