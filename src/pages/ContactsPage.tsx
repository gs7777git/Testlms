...
const ContactModal: React.FC<ContactModalProps> = ({ isOpen, onClose, contactToEdit, companies, onSave }) => {
  const initialContactState: Omit<Contact, 'id' | 'created_at' | 'updated_at' | 'org_id' | 'company_name'> = {
    first_name: '', last_name: '', email_primary: '', phone_work: '', phone_mobile: '', designation: '', company_id: null
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={contactToEdit ? "Edit Contact" : "Add New Contact"} size="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Select label="Company (Optional)" id="company_id" name="company_id" value={contactData.company_id || ''} onChange={handleChange} options={companyOptions} error={errors.company_id} disabled={isLoadingModal || companies.length === 0} placeholder="-- No Company --" />
