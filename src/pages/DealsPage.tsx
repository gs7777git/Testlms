...
export const DealsPage: React.FC = () => {
  ...
  const handleOpenModal = (deal: Deal | null = null, view: boolean = false) => {
    setEditingDeal(deal);
    setIsViewMode(view);
    setIsModalOpen(true);
  };
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard title="Total Deals" value={isDataLoading && !reportData.totalDealsCount ? <Spinner size="sm"/> : reportData.totalDealsCount} subtext={currentUserProfile?.role !== Role.ADMIN ? "My Deals" : "Org-wide"} />
        <StatCard title="Open Deals Value" value={isDataLoading && !reportData.openDealsValue ? <Spinner size="sm"/> : reportData.openDealsValue} formatAsCurrency />
        <StatCard title="Won Deals Value" value={isDataLoading && !reportData.wonDealsValue ? <Spinner size="sm"/> : reportData.wonDealsValue} formatAsCurrency />
        <StatCard title="Avg. Won Deal Size" value={isDataLoading && !reportData.avgWonDealSize ? <Spinner size="sm"/> : reportData.avgWonDealSize} formatAsCurrency />
      </div>
        ...
        <div className="lg:col-span-2 bg-white shadow overflow-x-auto rounded-lg">
            ...
            <tbody className="bg-white divide-y divide-secondary-200">
                {isDataLoading && deals.length > 0 ? (
                    ...
                    <tr><td colSpan={6} className="px-6 py-12 text-center text-sm text-secondary-500">No deals found.</td></tr>
                ) : (
                    ...
                    <tr key={deal.id} className="hover:bg-secondary-50">
                        ...
                        <Button variant="outline" size="sm" onClick={() => handleOpenModal(deal, true)} aria-label={`View ${deal.deal_name}`}><EyeIcon className="h-4 w-4" /></Button>
                        {(hasRole(Role.ADMIN) || deal.created_by_user_id === currentUserProfile?.id) &&
                            <Button variant="outline" size="sm" onClick={() => handleOpenModal(deal)} aria-label={`Edit ${deal.deal_name}`}><EditIcon className="h-4 w-4" /></Button>
                        }
                        {hasRole(Role.ADMIN) &&
