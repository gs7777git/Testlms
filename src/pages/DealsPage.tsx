...
import { PlusIcon, EditIcon, EyeIcon, DeleteIcon, TagIcon } from '@/components/common/Icons';
...
export const DealsPage: React.FC = () => {
  ...
  const filteredDeals = useMemo(() => {
    return deals.filter(deal => {
      ...
      const searchMatch =
        ...
        (deal.contact_name && deal.contact_name.toLowerCase().includes(searchTermLower));
  ...
  const statusOptionsForFilter = [{ value: '', label: 'All Statuses' }, ...DEAL_STATUS_OPTIONS.map(s => ({ value: s, label: s }))];
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <h1 className="text-3xl font-bold text-secondary-900 flex items-center">
            <TagIcon className="h-8 w-8 mr-3 text-primary-600"/> Deals & Quotations
        </h1>
      ...
      {leads.length === 0 && !pageLoading && (
         <p className="mb-4 p-3 bg-yellow-50 border border-yellow-200 text-yellow-700 rounded-md text-sm">
            Please add leads first to be able to create deals. You can create a deal from a Lead's detail view or select a lead when creating a new deal here.
        </p>
      )}

      {/* Summary Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard title="Total Deals" value={reportData.totalDealsCount} />
        <StatCard title="Open Deals Value" value={reportData.openDealsValue} formatAsCurrency={true} />
        <StatCard title="Won Deals Value" value={reportData.wonDealsValue} formatAsCurrency={true} />
        <StatCard title="Avg. Won Deal Size" value={reportData.avgWonDealSize} formatAsCurrency={true} />
      </div>

      {/* Chart */}
      <div className="mb-8">
        <BarChartPlaceholder title="Deals by Status (Count & Value)" data={dealsByStatusChartData} isLoading={isDataLoading && !dealsByStatusChartData} />
      </div>
        ...
        <table className="min-w-full divide-y divide-secondary-200" aria-label="Deals Table">
          ...
          <tbody className="bg-white divide-y divide-secondary-200">
             {isDataLoading && deals.length > 0 ? (
              ...
              <tr><td colSpan={8} className="px-6 py-12 text-center text-sm text-secondary-500">No deals found.</td></tr>
            ) : (
              filteredDeals.map((deal) => (
                <tr key={deal.id} className="hover:bg-secondary-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-secondary-900">{deal.deal_name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-secondary-500">
                     <Link to={`/leads`} state={{ preselectLeadId: deal.lead_id, viewMode: true }} className="text-primary-600 hover:underline">
                        {deal.lead_name || deal.lead_id.substring(0,8)}
                    </Link>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-secondary-500">{deal.created_by_user_name || 'N/A'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-secondary-500">{new Date(deal.updated_at).toLocaleDateString()}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                    ...
                    {(hasRole(Role.ADMIN) || deal.created_by_user_id === currentUserProfile?.id) && 
      ...
      {isModalOpen && (
        <DealModal
          ...
          // lead prop can be omitted here, DealModal will fetch leads if needed for creation from DealsPage
        />
      )}
    </div>
  );
