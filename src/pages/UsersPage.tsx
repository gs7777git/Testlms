...
export const UsersPage: React.FC = () => {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    setPageLoading(true); // Indicate page loading during fetch
    try {
  ...
  const handleSaveUser = async (
    ...
    // Page loading for table is managed by fetchData after successful save.
    // UserModal itself handles its internal loading state.
    try {
      if (isNew) {
      ...
      // Let the modal display the error by re-throwing it.
      throw error; 
    }
  };
  ...
  const handleDeleteUser = async (userToDelete: UserProfile) => {
    ...
    if (window.confirm(`Are you sure you want to delete user ${userToDelete.full_name}? This action will delete their profile. Deleting their authentication record requires a backend function with admin privileges and is not handled client-side.`)) {
        ...
        try {
            ...
            setPageLoading(false); // Ensure loading stops on error
        }
    }
  };
