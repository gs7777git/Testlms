...
export const TasksPage: React.FC = () => {
  ...
  const fetchData = useCallback(async (options?: {isInitialLoad?: boolean}) => {
    ...
    try {
      ...
      if (isInitialLoad) { // Only fetch these on initial load to avoid refetching static-ish data too often
  ...
  }, [currentUserProfile]); // Removed `companies`, `contacts`, `leads` from deps to avoid re-fetching them on simple refresh
  ...
  }, [currentUserProfile, fetchData, pageLoading]);
  ...
  const handleSaveTask = async (
    ...
    try {
      ...
      // Keep isDataLoading true if modal stays open on error, or set to false if that's preferred UX
      throw error; // Re-throw to be caught in modal if needed
    } finally {
        setIsDataLoading(false);
    }
  };
