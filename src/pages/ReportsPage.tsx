...
export const StatCard: React.FC<{ title: string; value: string | number | undefined | React.ReactNode; subtext?: string; formatAsCurrency?: boolean; icon?: React.ReactNode; color?: string; linkTo?: string; linkState?: any; }> = ({ title, value, subtext, formatAsCurrency = false, icon, color = 'text-primary-700', linkTo, linkState }) => {
    let displayValue: string | number | React.ReactNode = <Spinner size="sm"/>; // Default to spinner if value is processing
    if (React.isValidElement(value)) { // If value is already a ReactNode (e.g., spinner)
        displayValue = value;
    ...
    } else if (value === null || value === undefined) { // Explicitly handle null/undefined after checking for ReactNode
        displayValue = '--';
    }
...
export const ReportsPage: React.FC = () => {
  ...
  }, [profile, isLoading]); // Added isLoading to deps to re-evaluate if profile becomes available later
