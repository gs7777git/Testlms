...
export const LeadsPage: React.FC = () => {
  ...
  const fetchData = useCallback(async (options?: { isInitialLoad?: boolean }) => {
    ...
    if (isInitialLoad) {
        setPageLoading(true); // Full page spinner
    } else {
        setIsDataLoading(true); // Inline/subtle spinner or loading indicator
    }
  ...
  useEffect(() => {
    ...
    } else if (!currentUserProfile && !pageLoading) { // Ensure pageLoading stops if profile is null after initial check
      setPageLoading(false);
      setIsDataLoading(false);
      setLeads([]);
      setUsers([]);
    }
  }, [currentUserProfile, fetchData]); // Removed pageLoading from deps, it's managed by fetchData
  ...
  const handleSaveLead = async (leadFormData: Omit<Lead, 'id' | 'created_at' | 'updated_at' | 'owner_name' | 'org_id' | 'company_name' | 'contact_name' | 'activities' | 'follow_ups' | 'deals'> | Lead) => {
    ...
    try {
      ...
      // Keep isDataLoading true if error, or set to false if that's preferred UX for errors
    } finally {
        setIsDataLoading(false); // Always set to false after operation
    }
  };
      ...
      <div className="bg-white shadow overflow-x-auto rounded-lg relative">
        ...
        <table className="min-w-full divide-y divide-secondary-200" aria-label="Leads Table">
          ...
          <tbody className="bg-white divide-y divide-secondary-200">
           {isDataLoading && leads.length === 0 ? ( // Show spinner if initial data load is happening (no leads yet)
