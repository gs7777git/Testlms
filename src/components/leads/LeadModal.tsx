...
export const LeadModal: React.FC<LeadModalProps> = ({ isOpen, onClose, leadToEdit, onSave, viewMode = false }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      let dataToSave: Parameters<typeof onSave>[0];

      if (leadToEdit?.id && leadToEdit.org_id && leadToEdit.created_at && leadToEdit.updated_at) {
        // Editing an existing lead
        dataToSave = {
          ...leadToEdit, // Start with all existing lead data
          ...leadData,   // Override with form-editable fields
