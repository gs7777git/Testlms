...
export const TicketModal: React.FC<TicketModalProps> = ({
  isOpen, onClose, ticketToEdit, viewModeInitial = false, allUsers
}) => {
  ...
  const resetForm = useCallback(() => {
    ...
    setErrors({});
    setIsEditing(!viewModeInitial || !ticketToEdit); // Reset editing state based on initial mode and if a ticket is being edited
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (ticketToEdit) {
        await ticketService.updateTicket(ticketToEdit.id, payload, ticketToEdit);
      } else {
        await ticketService.addTicket(payload, currentUserProfile.org_id, currentUserProfile.id);
      }
      onClose(); 
    } catch (error: any) {
  ...
  const canEditThisTicket = hasRole(Role.ADMIN) || 
                              ...
                              !ticketToEdit; // Can always edit if it's a new ticket
