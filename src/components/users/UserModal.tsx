...
export const UserModal: React.FC<UserModalProps> = ({ isOpen, onClose, userToEdit, onSave, allUsers }) => {
  ...
  const validate = (): boolean => {
    ...
    if (formData.parent_user_id && formData.parent_user_id === userToEdit?.id) { // Check against userToEdit.id
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="pt-5">
          <div className="flex justify-end space-x-3">
            ...
            <Button type="submit" isLoading={isLoading} disabled={isLoading}>{isNewUser ? 'Create User' : 'Save Changes'}</Button>
          </div>
        </div>
      </form>
    </Modal>
