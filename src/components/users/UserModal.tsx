...
export const UserModal: React.FC<UserModalProps> = ({ isOpen, onClose, userToEdit, onSave }) => {
  ...
  const handleClose = () => {
    // Reset state when modal is closed, not just on open/prop change
    ...
    <Modal isOpen={isOpen} onClose={handleClose} title={modalTitle} titleId={MODAL_TITLE_ID} size="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Input
          ...
          onChange={handleChange}
          error={errors.email}
          required={isNewUser} // Email is only required for new users
          disabled={isLoading || !isNewUser} // Email field is disabled for existing users
        ...
        {(isNewUser || showPasswordFields) && (
          <div id="password-fields-group" className="space-y-4">
            <Input
              ...
              required={isNewUser} // Required for new user
              disabled={isLoading}
            ...
            <Input
              ...
              required={isNewUser && !!formData.passwordInput} // Required if password is set for new user
              disabled={isLoading}
              autoComplete="new-password"
            />
          </div>
