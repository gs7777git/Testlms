...
export const TaskModal: React.FC<TaskModalProps> = ({
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if ((name === 'assigned_to_user_id' || name === 'related_lead_id' || name === 'related_company_id' || name === 'related_contact_id' || name === 'due_date') && value === '') {
        processedValue = null;
    }
  ...
  const handleAddComment = async () => {
    ...
    try {
      await taskService.addTaskComment({ // user_id is handled by the service method now
  ...
  const renderDetailsTab = () => (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Select label="Assigned To" id="assigned_to_user_id" name="assigned_to_user_id" value={taskData.assigned_to_user_id} onChange={handleChange} options={userOptions} error={errors.assigned_to_user_id} required disabled={isLoading || !isEditing || allUsers.length === 0} placeholder={allUsers.length === 0 ? "No users available" : "-- Select Assignee --"} />
        <Input label="Due Date (Optional)" id="due_date" name="due_date" type="date" value={taskData.due_date || ''} onChange={handleChange} disabled={isLoading || !isEditing} />
      </div>
      
      <h4 className="text-md font-medium text-secondary-700 pt-2">Related Entities (Optional)</h4>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Select label="Lead" id="related_lead_id" name="related_lead_id" value={taskData.related_lead_id || ''} onChange={handleChange} options={leadOptions} disabled={isLoading || !isEditing || allLeads.length === 0} placeholder="None" />
          <Select label="Company" id="related_company_id" name="related_company_id" value={taskData.related_company_id || ''} onChange={handleChange} options={companyOptions} disabled={isLoading || !isEditing || allCompanies.length === 0} placeholder="None" />
          <Select label="Contact" id="related_contact_id" name="related_contact_id" value={taskData.related_contact_id || ''} onChange={handleChange} options={contactOptions} disabled={isLoading || !isEditing || allContacts.length === 0} placeholder="None" />
      </div>
  ...
  const renderCommentsTab = () => (
    <div className="space-y-4">
      <div>
        <Textarea label="New Comment" id="newComment" name="newComment" value={newComment} onChange={(e) => setNewComment(e.target.value)} rows={2} disabled={isSubmittingComment} />
        <Button onClick={handleAddComment} isLoading={isSubmittingComment} disabled={!newComment.trim() || isSubmittingComment} size="sm" leftIcon={<PlusIcon className="h-4 w-4"/>} className="mt-2">Add Comment</Button>
        {errors.comments && <p className="text-sm text-red-600 mt-1">{errors.comments}</p>}
      </div>
      <h4 className="text-md font-semibold text-secondary-700">Comment History</h4>
      {isCommentsLoading ? <Spinner /> : (
        comments.length > 0 ? (
          <ul className="space-y-3 max-h-60 overflow-y-auto p-1 pr-2">
            {comments.map(comment => (
              <li key={comment.id} className="p-3 border rounded-md bg-secondary-50 shadow-sm">
