...
import { DashboardIcon, LeadsIcon, UsersIcon, ReportsIcon, SettingsIcon, LogoutIcon, XMarkIcon, MenuIcon } from '@/components/common/Icons';
import { Button } from '@/components/common/Button';

// Navigation updated for simplified Admin/User roles from Supabase
const initialNavigation: Omit<NavigationItem, 'current'>[] = [
  ...
  { name: 'Users', href: '/users', icon: UsersIcon, roles: [Role.ADMIN] }, // Only Admin
  { name: 'Reports', href: '/reports', icon: ReportsIcon, roles: [Role.ADMIN] }, // Only Admin
...
export const PageShell: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const { profile, logout, hasRole } = useAuth(); // Use 'profile' from useAuth (Supabase user profile)
  const location = useLocation();
  const navigate = useNavigate();

  const handleLogout = async () => {
    await logout(); // Calls Supabase logout
    navigate('/login'); 
  };

  const navigation = initialNavigation
    .filter(item => !item.roles || hasRole(item.roles)) // Filter based on profile.role
    ...
    <>
      <div className="flex flex-shrink-0 items-center px-4 h-16 bg-primary-700">
         <img
            ...
            src="https://tailwindui.com/img/logos/mark.svg?color=white" // Replace with your logo or remove
            alt={APP_NAME}
          />
      ...
      <div className="mt-5 flex flex-grow flex-col">
        <nav className="flex-1 space-y-1 px-2 pb-4">
          {navigation.map((item) => (
            <Link
              ...
              onClick={() => sidebarOpen && setSidebarOpen(false)} // Close mobile sidebar on nav
            >
              <item.icon
                className={`
       ...
       {profile && ( // Display user info if profile (from Supabase) exists
        <div className="flex flex-shrink-0 border-t border-primary-600 p-4">
            <div className="flex-shrink-0 group block">
                ...
                <div className="ml-3">
                    ...
                    <p className="text-xs font-medium text-primary-200 group-hover:text-white capitalize">{profile.role}</p>
                </div>
                </div>
            </div>
        </div>
