...
export const LeadModal: React.FC<LeadModalProps> = ({ isOpen, onClose, leadToEdit, onSave, viewMode = false, initialTab = 'details' }) => {
  ...
  const [activeTab, setActiveTab] = useState<ActiveLeadModalTab>(initialTab);
  ...
  useEffect(() => {
    if (isOpen) {
        const currentInitialTab = initialTab || 'details';
        ...
        if (leadToEdit) {
            ...
            if (leadToEdit.id && currentUserProfile?.org_id) {
                 if (currentInitialTab !== 'details') {
                    fetchRelatedData(leadToEdit.id, currentUserProfile.org_id, currentInitialTab);
                }
            }
  ...
  }, [leadToEdit, isOpen, currentUserProfile?.org_id, currentUserProfile?.id, viewMode, fetchRelatedData, resetFormStates, initialTab]);
